function [XYZ_mm, XYZ_src_vx] = WFK_ea_map_coords(varargin)
...
%
% WFK_ea_map_coords is a copy of ea_map_coords.m so I can modify or at
% least add comments as I figure out how to use this.
%    -- Bill Kaemmerer, August 23, 2016.
%

try
    XYZ_vx=varargin{1};  % MNI coordinates as voxels indices
end
try
    trg=varargin{2};     % the target image (MRI in MNI space) - OK to leave blank
end
try
    xfrm=varargin{3};    % the y_ ... deformation field file
end
try
    src=varargin{4};     % the source image (MRI from patient)
end
...
for i = 1:size(coord, 2)
    ind = inds(:, i);
    %
    %  WFK -- BUG ?!? spm_sample_vol returns voxel 
    %  VALUES, not transformed coordinates for the given coords passed to
    %  it.
    %     Bill Kaemmerer  August 23, 2016.
    %
