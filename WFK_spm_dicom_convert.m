function out = WFK_spm_dicom_convert(handles,hdr,opts,root_dir,format,out_dir)
...
% WFK edited version of spm_dicom_convert to enable user to get 
% feedback on the progress of file processing via a GUI.
%  -- Bill Kaemmerer (WFK)  - June 10, 2016.
%

%-Input parameters
%--------------------------------------------------------------------------
if nargin<3, opts     = 'all';  end
if nargin<4, root_dir = 'flat'; end
if nargin<5, format   = spm_get_defaults('images.format'); end
if nargin<6, out_dir  = pwd;    end
...
if ~isempty(guff)
    warning('off','backtrace');  %  WFK June 13, 2016
...
if (strcmp(opts,'all') || strcmp(opts,'standard')) && ~isempty(standard)
    fstd = convert_standard(handles,standard,root_dir,format,out_dir);
end
...
%==========================================================================
function fnames = convert_standard(handles,hdr,root_dir,format,out_dir)
hdr = sort_into_volumes(hdr);
fnames = cell(length(hdr),1);
set(handles.ProcessingStepText,'String','Writing nii Files');
set(handles.TotalCountText,'String',sprintf('%d',length(hdr)  ));
pause on;
for i=1:length(hdr)
    set(handles.CurrentText,'String',sprintf('%d',i));
    pause(0.005); % to give the GUI a chance to update -- WFK
