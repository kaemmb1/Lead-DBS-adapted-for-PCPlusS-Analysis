function WFK_ea_apply_normalization(handles,outRoot,patientName,anatFileName)
...
% Andreas Horn
%
% Code adapted by Bill Kaemmerer (WFK) for the Seeking Beta project, 
%  June 21, 2016

% 
% options.earoot = outRoot;
...
options.prefs.tranii_unnormalized = 'anat.nii';  % swranat.nii will be renamed... 
options.prefs.tranii =              'lanat.nii'; % ... lanat.nii
options.prefs.ctnii_coregistered  = 'rpostop_ct.nii'; % swrrpostop_ct.nii will be renamed...
options.prefs.ctnii =               'lpostop_ct.nii'; % ... lpostop_ct.nii 
options.prefs.gtranii =             'glanat.nii';
options.prefs.gctnii  =             'glpostop_ct.nii';
    
postops={'anat.nii','rpostop_ct.nii'};
nSteps = length(postops);
...
for postop=1:nSteps
    if exist([options.root,options.patientname,filesep,postops{postop}],'file')
        ...
        jobs{1}=matlabbatch;
        set(handles.ProcessingStepText,'String',sprintf('Normalizing: %s',postops{postop}));
        set(handles.CurrentText,'String',sprintf('File %d',postop));
        set(handles.TotalCountText,'String',sprintf('%d',2*nSteps));
        pause on;
        pause(0.5);
        cfg_util('run',jobs);
...
for postop=1:nSteps
    if exist([options.root,options.patientname,filesep,postops{postop}],'file') 
        ...
        jobs{1}=matlabbatch;
        set(handles.ProcessingStepText,'String',sprintf('Normalizing: %s',postops{postop}));
        set(handles.CurrentText,'String',sprintf('File %d',postop+2));
        set(handles.TotalCountText,'String',sprintf('%d',2*nSteps));
        pause on;
        pause(0.5);
        spm_jobman('run',jobs);
...
set(handles.ProcessingStepText,'String','Done applying normalization.');
